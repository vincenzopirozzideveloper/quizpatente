...
class QuizService
{
    ...
    const MAX_ERRORS_IN_QUIZ = 3; // Aggiornato a 3 errori massimi
    
    /**
     * Tipi di quiz disponibili
     */
    const QUIZ_TYPE_MINISTERIAL = 'ministerial';      // Quiz ministeriali predefiniti
    const QUIZ_TYPE_TOPIC = 'topic';                  // Quiz per argomento (automatico)
    const QUIZ_TYPE_ERRORS_REVIEW = 'errors_review';  // Ripasso errori (automatico)
    const QUIZ_TYPE_CUSTOM = 'custom';                // Altri tipi personalizzati
    
    /**
     * Genera un quiz ministeriale predefinito
     */
    ...
    /**
     * Ottiene i quiz ministeriali disponibili per l'utente
     */
    ...
    /**
     * Calcola la progressione dei quiz ministeriali
     */
    ...
    /**
     * Genera un quiz per argomento specifico (rimane automatico)
     */
    public function generateTopicQuiz(User $user, int $topicId): QuizSession
    {
        $topic = Topic::findOrFail($topicId);
        
        // Seleziona domande dall'argomento specifico
        ...
        // Se non ci sono abbastanza domande per l'argomento, riempi con domande casuali
    ...
    /**
     * Genera un quiz basato sugli errori dell'utente (rimane automatico)
     */
    public function generateErrorsReviewQuiz(User $user): QuizSession
    {
        // Recupera le domande con errori non ancora padroneggiate
        ...
        // Se non ci sono abbastanza errori, aggiungi domande random
        if ($questions->count() < self::QUESTIONS_PER_QUIZ) {
            $remainingCount = self::QUESTIONS_PER_QUIZ - $questions->count();
            
            // Prendi i topic delle domande con errori
            ...
            // Se ancora non bastano, aggiungi domande casuali
    ...
    protected function createQuizSession(
        ...
        return DB::transaction(function () use ($user, $type, $questions, $topicId, $metadata, $ministerialQuizId) {
            ...
            // Crea le risposte vuote per ogni domanda
    ...
    public function getQuizTypeStats(User $user): array
    {
        ...
        // Statistiche quiz ministeriali predefiniti
        ...
        // Aggiungi la progressione dei quiz ministeriali
        $stats['ministerial'] = array_merge(
            $stats['ministerial'],
            $this->getMinisterialQuizzesProgress($user)
        );
        
        // Statistiche quiz per argomento
